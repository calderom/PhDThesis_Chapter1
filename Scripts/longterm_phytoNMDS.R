...
#input file
bioA <- read.csv("Data/Phyto_NMDSbio_transRA_2012_2020.csv", head = TRUE, check.names = FALSE, row.names = 1)
str(bioA)
...
#Species = expanded scores based on data
...
summary(bio.ano) #0.1385 (p = 0.001)
plot(bio.ano)
...
ggsave(path = "Figs", filename ="Fig9a.jpeg", width = 20, height = 15, units = "cm")
...
phytoNMDS <- plot_grid(a, b,
                       align="hv", axis="tblr", ncol = 1, labels =c("a","b"))
title <- ggdraw() + 
  draw_label("Phytoplankton and ciliates - community composition (2012-2020)",
             fontface = 'bold',
             x = 0,
             hjust = 0,
             size = 12
...
Figure17 <- plot_grid(title, phytoNMDS, ncol=1, rel_heights = c(0.1, 1))

ggsave(path = "Figures", filename ="Figure17.jpeg", width = 18, height = 24, units = "cm")
